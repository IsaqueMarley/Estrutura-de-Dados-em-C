...
bool inOrdem2(ARVORE_BINARIA *arv,ARVORE_BINARIA **arv2)
{
   int cont =0;
  if(arv==NULL) return false;
  inOrdem2(arv->esq,arv2);
  
  if (inserir(arv->item, arv2)){
	inserir(arv->item, arv2);
	cont++;
  }
	
  inOrdem2(arv->dir,arv2);
  if(cont ==0)
	return false;
  else
	return true;
...
bool uniao(ARVORE_BINARIA *arv1, ARVORE_BINARIA *arv2, ARVORE_BINARIA **arvU)
{
  ...
  if(inOrdem2(arv1,arvU) && inOrdem2(arv2,arvU))
  	return true;
  else
	return false;
...
int main()
{
   ...
   printf("\n");
  /* ARVORE_BINARIA *arv4 = NULL;
   inOrdem2(arv,&arv4);
   inOrdem(arv4, imprimir);
   inOrdem(arv, imprimir);*/
   printf("\n");
