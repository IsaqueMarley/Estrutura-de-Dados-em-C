...
void inverter(LISTA *l) {
  NO* itemAtual = (NO *)malloc(sizeof(NO));
  NO* itemAnt =(NO *)malloc(sizeof(NO));
  NO* itemProx = (NO *)malloc(sizeof(NO));

  itemAnt = l->cabeca;
  itemAtual = itemAnt->prox;

    
  while (itemAtual != l->cabeca) {
	
   // printf("chave: (%d,%s)\n", itemAtual->item.chave,itemAtual->item.valor);
    
      
    itemProx = itemAtual->prox;
    
    itemAtual->prox = itemAnt;
  
    itemAnt = itemAtual;
    
    itemAtual = itemProx;
    
    //printf("passei o loop\n");
    //printf("chave apos: %d\n", l->cabeca->prox->item.chave);

    
  }
  //printf("CABECA: %d\n", l->cabeca->prox->prox->prox->prox->prox->prox->item.chave);
  l->cabeca->prox = itemAnt;
}
