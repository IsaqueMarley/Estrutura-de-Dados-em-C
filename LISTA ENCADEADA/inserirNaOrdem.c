...
{
  NO *aux,*check, * novo = (NO*)malloc(sizeof(NO));
  novo->item = item;
  if(l->cabeca == NULL){
	l->cabeca = novo;
	l->cabeca->prox = NULL;
  ...
  else if ( item.chave <= l->cabeca->item.chave){
	...
	aux = l->cabeca;
 	while(aux->prox && item.chave >= aux->prox->item.chave){
		if (item.chave == aux->prox->item.chave || item.chave == aux->item.chave ){
			return false;
		}
		aux = aux->prox;	
         }
 	...
 	if(aux->prox){
		if (novo->item.chave == aux->prox->item.chave || novo->item.chave == aux->item.chave  )
	  		return false;
		novo->prox = aux->prox;
		aux->prox = novo;
	}
