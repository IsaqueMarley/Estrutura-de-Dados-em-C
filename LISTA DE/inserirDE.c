...
NO * aux, *novoItem = (NO*)malloc(sizeof(NO));
    ...
    if (tamanho(l) ==0){
    ...
    else if (l->cabeca->item.chave > novoItem->item.chave){
      novoItem->prox = l->cabeca;
      novoItem->ant = NULL;
      l->cabeca->ant = novoItem;
      l->cabeca = novoItem;
      ...
      aux = l->cabeca;
      while(aux->prox && novoItem->item.chave > aux->prox->item.chave)
        aux= aux->prox;
      if(aux->prox){ //verifica se ha um proximo
	if (novoItem->item.chave == aux->prox->item.chave || novoItem->item.chave == aux->item.chave  )
	  return false;
